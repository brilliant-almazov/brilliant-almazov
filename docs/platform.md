...
## 3. A control-plane registry

The single writer for *who is alive, where, and in what shape*: registration from every running
instance, signed heartbeats plus health pulls from the registry side, a read API for operational UIs, and
a signed reverse proxy for process control and scaling.

## 4. Contracts as artefacts
